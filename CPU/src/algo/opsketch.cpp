#include <algo/opsketch.h>
#include <murmur3.h>
#include <const.h>

OPSketch::OPSketch(uint32_t _B, double _T, double _mem, uint32_t _K)
: K(_K) {
    B = _B;
    T = _T;
    mem = _mem;

    sprintf(name, "OPSketch");
...
void OPSketch::status() {
    printf("[%s]\n", name);
    printf("total bucket: %d   mem: %.3lfKB   #Hash: %d    B: %u   T: %.3lfMbps\n", K*W, mem/1024, K, B, T*SEC_TO_MICROSEC/1024/1024*8);
}

std::string OPSketch::tag() {
    static char s[1000];
    sprintf(s, "OPSketch k=%d", K);
    return std::string(s);
}

Signal OPSketch::process(uint32_t x, uint32_t f, uint32_t t) {
